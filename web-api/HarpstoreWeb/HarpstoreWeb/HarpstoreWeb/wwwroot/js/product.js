            ...
            $.each(data.dataProductView, function (i, v) {
                $('#tbProduct tbody').append(
                    `<tr>
                        ...
                        <td><img src=' ${v.avataPath} ' width='100px' height='70px' /></td>
                        <td>${v.proTypeName}</td>
                        <td>${v.productPrice}</td>
                        <td>
                            <a href="javascripts:;" class="btn btn-success"
                                           onclick="product.get(${v.productId})">Edit</a> 
                            <a href="javascripts:;" class="btn btn-danger"
                                           onclick="product.delete(${v.productId})">Remove</a>
                        </td>
                    </tr>`
                )
            });
...
product.get = function (id) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#ProducerId').val(data.result.producerId);
            $('#ProTypeId').val(data.result.proTypeId);
            $('#ProductPrice').val(data.result.productPrice);
            $('#FullDescription').val(data.result.fullDescription);
            $('#ShortDescription').val(data.result.shortDescription);
...
product.save = function () {
    ...
    saveObj.AvataPath = $('#AvataPath').attr('src');
    //saveObj.AvataPath = "ádasdsad";
    saveObj.ProducerId = parseInt($('#ProducerId').val());
    saveObj.ProTypeId = parseInt($('#ProTypeId').val());
    saveObj.ProductPrice = parseFloat($('#ProductPrice').val());
    saveObj.ShortDescription = $('#ShortDescription').val();
    saveObj.FullDescription = $('#FullDescription').val();
    //if (!($('#ProductId').val())) {
    //    saveObj.ProductId = 0;
    //}
    //else{
...
product.uploadImage = function (input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            $('#AvataPath').attr('src', e.target.result);
            console.log(e.target.result);
        };
        reader.readAsDataURL(input.files[0]);
    }
};
product.openSaveProductModel = function () {
    product.reset();
    $('#saveProduct').modal('show');
};
// modified
...
$(document).ready(function () {
    producerId = $('#ProducerHiddenId').val();
    product.init();
});
            ...
            $.each(data,function(i,v){
                $('#tbProducts tbody').append(
                    "<tr>"+
                        ...
                        "<td> <i>"+ convertCurency(v.price) +"&#8363;"+"</i></td>" +
                        "<td>"+
                            "<a href='javascript:void(0);' title='edit student' onclick='product.edit("+ v.id +")'><i class='fa fa-edit'></i></a> " +
                            "<a href='javascript:void(0);' title='remove student' onclick='product.delete("+ v.id +")'><i class='fa fa-trash'></i></a> " +
        ...
        if($('#id').val() == 0) //create
        {
            ...
            $.ajax({
                ...
                success : function(data){
                    ...
                    bootbox.alert("Sản phẩm đã được thêm thành công!");
                    product.drawTable();
                }
            });
        }
            ...
            addObj.image = $('#Image').attr('src');
            addObj.price = $('#Price').val();
            addObj.id = parseInt($('#id').val());
    
            $.ajax({
                url: "http://localhost:3000/productObj/" + addObj.id,
                method : "PUT",
                data : JSON.stringify(addObj),
                dataType: "json",
                contentType: "application/json",
                success : function(data){
                    $('#addEditProduct').modal('hide');
                    bootbox.alert("Sản phẩm đã cập nhật thành công!");
                    product.drawTable();
                }
            });
        }
...
product.edit = function(id){
    $.ajax({
        url : "http://localhost:3000/productObj/" + id,
        method : "GET",
        contentType : "JSON",
        success : function(data){
            $("#Name").val(data.name);
